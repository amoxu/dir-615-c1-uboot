...
/* NickChou reduce image size
int  gunzip (void *, int, unsigned char *, unsigned long *);
*/
int  lzma_decode(unsigned char *in_buf, unsigned char* out_buf, unsigned int in_len, unsigned int out_len);
...
# error Unknown CPU type
	...
	case IH_COMP_NONE:
		...
		break;
//	case IH_COMP_GZIP:
//		printf ("GZIP   Uncompressing %s ... ", name);
...
#ifdef CONFIG_BZIP2
	case IH_COMP_BZIP2:
		printf ("   BZIP2 Uncompressing %s ... ", name);
		/*
...
#endif /* CONFIG_BZIP2 */
	case IH_COMP_LZMA:
                printf ("   LZMA Umcompressing %s ... ", name);
                i = lzma_decode((char *)data, (char*)ntohl(hdr->ih_load), len, &unc_len);
                if (i != 0) {
                        printf("lzma_decode_failed  \n");
                        udelay(100000);
                        do_reset (cmdtp, flag, argc, argv);
                }else
                        printf("   Image loaded from %p-%p\n ",(char*)ntohl(hdr->ih_load), (char*)ntohl(hdr->ih_load)+unc_len);
                break;
	default:
		if (iflag)
			enable_interrupts();
