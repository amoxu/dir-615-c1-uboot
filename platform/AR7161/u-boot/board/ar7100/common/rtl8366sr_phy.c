...
//for AP94
#define I2CMASTER_SDA_PIN 5//gpio 5 //ETH_I2C_SDA     GPIO5 (A21)
#define I2CMASTER_SCL_PIN 7//gpio 7 //ETH_I2C_SCK     GPIO7 (A19)
...
uint32_t rtl8366s_initChip(void)
{
	...
	{
 	 case 0x0000:	
		DEBUG_MSG("rtl8366s_initChip: 0x0000\n");
		...
		break;
 	 case 0x6027:	
 	 	DEBUG_MSG("rtl8366s_initChip: 0x6027\n");
...
rtl8366sr_phy_setup(int ethUnit)
{
    ...
    printf("rtl8366sr_phy_setup  ethUnit=%x\n", ethUnit); 
	...
	for (phyUnit=phyStart; phyUnit < phyEnd; phyUnit++) {
        ...
        if (rtl8366sr_phy_is_link_alive(phyUnit)) {
        	DEBUG_MSG("rtl8366sr_phy_setup: phyUnit=%x is link\n",phyUnit);
            liveLinks++;  
        }  
    }  
#if GET_INFO
...
rtl8366sr_phy_speed(int ethUnit)
{
   ...
   DEBUG_MSG("rtl8366sr_phy_speed: ethUnit:%d\n",ethUnit);
    ...
    for (phyUnit=phyStart; phyUnit < phyEnd; phyUnit++) {       
        if (rtl8366sr_phy_is_link_alive(phyUnit)) {
        	DEBUG_MSG("rtl8366sr_phy_speed: phyUnit=%x is link\n",phyUnit);
			uint32_t link_status_giga;
			uint32_t local_cap,remote_cap,common_cap;
        	...
        	DEBUG_MSG("rtl8366sr_phy_speed: phyUnit=%x is no link\n",phyUnit);
        }	
    }    
#endif    
    return _10BASET;
#else
#ifdef CFG_SP1000
	DEBUG_MSG("rtl8366sr_phy_speed: SP1000\n");
	return _1000BASET;
#else
	DEBUG_MSG("rtl8366sr_phy_speed: SP100\n");
	return _100BASET;
#endif	
#endif    
}
