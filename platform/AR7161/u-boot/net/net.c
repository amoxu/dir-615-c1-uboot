...
#undef ARP_TIMEOUT_COUNT
#define ARP_TIMEOUT_COUNT 1 
...
#ifndef AP99
#ifdef CFG_BOOTP_FW_UPGRADE
			*(volatile int *)(0xb8040008) &=~(1<<1);//Status abmber light on
			*(volatile int *)(0xb8040008) &=~(1<<2);//Status blue light on
#endif
#endif
			BootpRequest ();
			break;
...
#  endif /* CFG_FAULT_ECHO_LINK_DOWN, ... */
		...
		case NETLOOP_SUCCESS:
			...
			eth_halt();
#else
			if (protocol == BOOTP) // success, then chk hw id do fw upgrade
			{ 
				char *phwid = (void *)(CFG_LOAD_ADDR + NetBootFileXferSize -24);
				...
				if(strncmp(imageHWID,(void *)HWID_LOCATION,HWID_LEN) == 0) 
				{
	        		if (flash_erase_t(CFG_FLASH_SECTOR_SIZE, 0x3, 0x65) == 0) { // from 0x3 to 0x65
						printf("\nErase flash done!\n");
						write_buff(NULL, (void *)CFG_LOAD_ADDR, KERNEL_LOCATION, NetBootFileXferSize);
						do_reset(0, NULL);
					}
				} else
					printf("\n Kernel Hardware ID Check Error\n");
			}
			if (strcmp("eth1", eth_get_name()) == 0)
			{
				DEBUG_MSG("change form eth1 to eth0!!\n");
				eth_halt();
				eth_try_another(0);
				eth_init(gd->bd);
				eth_set_current();
				NetOurIP = getenv_IPaddr("ipaddr");
				DEBUG_MSG("NetOurIP=%lx\n", NetOurIP);
			}
			*(volatile int *)(0xb8040008) |= (1<<1);//Status abmber light off
			*(volatile int *)(0xb8040008) |= (1<<2);//Status blue light off
#endif
		...
		case NETLOOP_FAIL:
			...
			*(volatile int *)(0xb8040008) |= (1<<1);//Status abmber light off
			*(volatile int *)(0xb8040008) |= (1<<2);//Status blue light off
//			*(volatile int *)(0xb8040008) |= (1<<6);//Internet amber light off
#endif
#endif 
			return (-1);
		}
...
#else
		if (strcmp("eth0", eth_get_name()) == 0)
		{
			NetOurIP = getenv_IPaddr("ipaddr");
			printf("got ARP, fail 5??\n");
		}
#endif
