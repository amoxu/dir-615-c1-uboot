...
/*
	Flash Partition Allocation for 8M
	Date:	2010-05-04
	Name:	Jimmy Huang
	Related Files:
		apps/sutil/project.h
		u-boot/include/ap98.h (CONFIG_BOOTARGS, )
		u-boot/httpd/cgi.c  (KERNEL_LOCATION)

	Type	Start Address	End Address		Size
	U-Boot	0x00000000		0x00030000		 192	kbytes
	nvram	0x00030000		0x00040000		  64	kbytes
	linux	0x00040000		0x001c0000		1536	kbytes
	rootfs	0x001c0000		0x007b0000		6080	kbytes
	LANG	0x007b0000		0x007e0000		 192	kbytes
	MAC		0x007e0000		0x007f0000		  64	kbytes
	ART		0x007f0000		0x00800000		  64	kbytes
	
#define CONFIG_BOOTARGS     "console=ttyS0,115200 root=31:03 rootfstype=squashfs,jffs2 init=/sbin/init mtdparts=ar7240-nor0:192k(u-boot),64k(nvram),1536k(linux),5120k(rootfs),896k(PLC),64k(PIB),192k(LANG),64k(MAC),64k(ART)"
*/
...
//#define CONFIG_BOOTARGS     "console=ttyS0,115200 root=31:03 rootfstype=squashfs,jffs2 init=/sbin/init mtdparts=ar7240-nor0:256k(u-boot),64k(nvram),1536k(linux),5056k(rootfs),896k(PLC),64k(PIB),64k(MAC),192k(LANG),64k(ART)"
#define CONFIG_BOOTARGS     "console=ttyS0,115200 root=31:03 rootfstype=squashfs,jffs2 init=/sbin/init mtdparts=ar7240-nor0:192k(u-boot),64k(nvram),1536k(linux),6080k(rootfs),192k(LANG),64k(MAC),64k(ART)"
#else
...
#define BOOT_IMG_UPPER_BOUND	192 * 1024 //for upgrade bootloader in backup mode
...
#else
#define CONFIG_COMMANDS (( CONFIG_CMD_DFL & ~CFG_CMD_NFS | CFG_CMD_NET | CFG_CMD_MII | CFG_CMD_PING))
#endif
